...
class LineManageActivity : TranslateStatusBarActivity() {
    ...
    override fun initClick() {
        adapter.setOnItemClickListener { view, position ->
            if (lines[position].status == -1)
                ...
                startActivityForResult<LineManageDetailActivity>(1, "id" to lines[position].id, "title" to lines[position].lineName)
        }
    }

    private val lines = arrayListOf<Line>()
    private val adapter by lazy {
        LineManageAdapter(lines)
    }
    private lateinit var rootView: View
    private lateinit var mSwipeRefreshRecyclerLayout: SwipeRefreshRecyclerLayout
    override fun initView() {
        title = "线路管理"
        titleBar.addRightButton("添加线路", onClickListener = View.OnClickListener {
            startActivityForResult<AddLineActivity>(1)
        })
    ...
    private var page = 1
    private fun getData() {
        HttpManager.getManageLineList(userId, page).request(this, success = { _, data ->
